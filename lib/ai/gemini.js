...
/**
 * Stream text from a prompt as an async generator of incremental string chunks.
 *
 * Used by the Coach chat surface (M5) so the synthesized reply renders tokens
 * as they arrive. Yields each chunk's text exactly as the SDK provides it
 * (caller concatenates). Falls back to a single non-streamed `generateText`
 * yield if the cached model rejects streaming for any reason, so callers can
 * always `for await ... yield` regardless of provider quirks.
 *
 * @param {string} prompt
 * @param {object} [opts]
 * @param {AbortSignal} [opts.signal] - aborts the stream mid-generation
...
export async function* generateTextStream(prompt, { signal } = {}) {
  ...
  // Last-resort: fall back to a single non-streamed generation so the chat
  // surface always gets a reply even if streaming is unsupported for the
  // resolved model.
  const text = await generateText(prompt);
  if (text) yield text;
}