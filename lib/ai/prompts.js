...
`;
  // Dream Company Mode — when a target company is set, tailor the questions to
  // its known interview themes + values. Append-only so `company = null`
  // produces the byte-identical baseline string.
  if (company) {
...
export const improvementTipPrompt = (industry, wrongQuestionsText, company = null) => {
  // Dream Company Mode — prepend the target-company context only when set.
  // `company = null` → empty prefix → byte-identical baseline string.
...
`;
  // Dream Company Mode — when a target company is supplied, weight the
  // recommendations toward its values, hiring bar, and expected skills.
  // Append-only so `company = null` is byte-identical to the baseline.
  if (company) {
    s += `
...
`;
  // Dream Company Mode — when a target company is set, steer the interviewer
  // toward its known themes + famous questions. Append-only so `company = null`
...
`;
  // Dream Company Mode — when a target company is set, score against its
  // hiring bar + values and emit an additive `companyFit` (0-100). Append-only
  // so `company = null` is byte-identical to the baseline.
  if (company) {
    s += `
...
// ── Dream Company Mode ─────────────────────────────────────────────
// Company-grained market + interview knowledge. Mirrors industryInsightsPrompt
// but is company-scoped. Generated on-demand (getOrGenerateCompanyProfile) +
// refreshed by the weekly `generateCompanyProfiles` Inngest cron. Used by the
// salary section (rendered directly) and as context for the personalized plan.
...
/**
 * The core Dream Company artifact: one AI call produces all 7 personalized
 * areas for a user targeting a company. EVERY item carries a `why` grounded in
 * BOTH the candidate's background AND the company's bar/values.
 *
 * Salary is deliberately NOT in this JSON — it is rendered directly from the
 * CompanyProfile (rule-based, no invented AI numbers).
 *
 * `userProfile` = { experience, skills[], bio, industry, activeGoal:{targetRole,targetLevel,timeframe}|null, recentMockWeaknesses[] }
 * `companyProfile` = the CompanyProfile row (displayName, values, bar, topSkills, recommendedSkills, interviewThemes, famousQuestions).
 */
