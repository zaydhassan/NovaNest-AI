/**
 * Career Twin builder (M8) — gathers the twin's source data (memory, resume,
 * GitHub, recent mocks, applications) and runs the `twinBuild` prompt through
 * Gemini to produce the persisted profile JSON.
 *
 * Used by the Inngest `rebuild-career-twin` job. Server-only.
 */
...
/**
 * Gather everything the twin is built from. Read-only; `client = tx ?? db`.
 * Memory is grouped by type so the prompt can reason per-category.
 */
export async function gatherTwinSources(userId, client = db) {
  ...
  // Group memories by type, capped per type so no one category dominates.
  const grouped = {};
...
/**
 * Run the twin-build prompt and normalize the result into a persisted shape.
 * Never throws — returns `{ twin, error }` so the job can persist a clean
 * failed state.
 */
...
/** Coerce the model JSON into a stable, renderable twin shape. */
