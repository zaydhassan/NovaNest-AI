/**
 * Career Intelligence Engine — produces all 8 career-intelligence metrics, each
 * as {score, level, levelBlurb, evidence[], why, how, whatToImprove} (+ optional
 * chart payload). The score is ALWAYS a deterministic, faithful number derived
 * from the user's own data; the explanation is assembled from the SAME data
 * that produced the score, so every claim in `why`/`how`/`whatToImprove` cites a
 * concrete data point in `evidence[]`. This is the only design that satisfies
 * the three product constraints:
 *   1. "Everything should explain Why / How / What to improve"
 *   2. "Never show meaningless percentages."  (no AI-invented scores)
 *   3. "Every score must have supporting evidence."
 *
 * 4 metrics REUSE the existing pure scoring functions unchanged — they are
 * imported and called as-is, then wrapped by an *assembler* that layers the
 * evidence + why/how/what on top of their returned breakdown (the source
 * functions are NEVER edited → no-regression):
 *   - Career Health      ← computeHealthScore  (lib/career/career/career-engine.js)
 *   - Resume              ← computeNovaScore resume pillar + Resume.atsScore/feedback
 *   - Interview Readiness ← computeInterviewReadiness (lib/career/career/readiness.js)
 *   - Skill Growth        ← computeSkillGrowth  (lib/career/career/skill-growth.js)
 *
 * 4 metrics are NEW pure functions defined here:
 *   - Learning Velocity, Application Success Rate, Consistency, Productivity
 *
 * Pure — no DB access. `actions/intelligence.js` gathers the rows and passes
 * them in (mirrors lib/nova-score.js + lib/career/career/career-engine.js).
 *
...
/** Defensive JSON parse for the Resume.feedback column (stringified JSON). */
...
// The canonical metric shape is assembled by these builders so every metric
// carries the same fields. `chart` is optional (only where a chart adds signal).
...
// ── level tables (4 tiers each, mirroring READINESS_LEVELS / HEALTH_LEVELS) ──
...
// ── 1. Career Health (reuse computeHealthScore) ─────────────────────────
const PILLAR_LABELS = {
  resume: "Resume",
  interview: "Interview",
...
function assembleCareerHealth(health) {
  ...
  // Rank pillars weakest-first (excluding the composite itself).
...
// ── 2. Resume Score (reuse computeNovaScore resume pillar + Resume.atsScore/feedback) ─
...
function assembleResumeScore({ resume, novaBreakdown }) {
  ...
  // Prefer the ATS score when it exists (the most concrete resume metric); fall
  // back to the NovaScore resume pillar (section-completeness heuristic).
...
// ── 3. Interview Readiness (reuse computeInterviewReadiness) ───────────
const SUB_LABELS = {
...
function assembleReadiness(readiness, topImprovement) {
  ...
  // Find the weakest scored sub (ignore 0 = no data for that sub).
...
function computeLearningVelocity({ sessions = [], topics = [], weeks = 8 }) {
  ...
  // Bucket sessions into the last `weeks` weeks (oldest → newest).
  ...
  // buckets[0] = this week (newest), buckets[weeks-1] = oldest. Reverse for chronological.
  const chrono = [...buckets].reverse();
  const recentAvg = avg(chrono.slice(-4)); // last 4 weeks
  const priorAvg = avg(chrono.slice(0, weeks - 4)); // prior weeks
  const trend =
...
// ── 5. Application Success Rate (NEW) ───────────────────────────────────
const STAGE_WEIGHT = {
  SAVED: 0.05,
  APPLIED: 0.2,
  SCREENING: 0.4,
...
function computeApplicationSuccessRate({ applications = [] }) {
  ...
  // Find the biggest leak: the stage with the most apps that aren't advancing past it.
  const leakStage = STAGE_ORDER.slice(0, 4) // SAVED..INTERVIEW
...
// ── 6. Skill Growth (reuse computeSkillGrowth) ──────────────────────────
...
function computeConsistency({ streak = 0, lastActiveAt = null, activityDays = [] }) {
  ...
  // Longest consecutive run across all distinct active days.
  let longestStreak = 0;
  let run = 0;
  let prevTs = null;
...
// ── 8. Productivity (NEW) ────────────────────────────────────────────────
// Maps gathered per-source counts → the XP_BY_EVENT keys they correspond to.
...
// XP values for the "sparse high-value type" recommendation come from the
// canonical lib/gamify.js XP_BY_EVENT map (single source — no drift risk). The
// score itself uses the user's real xp total, not these — so this only weights
// the advice text. Aliased to EVENT_XP at the use sites below.
...
function computeProductivity({ xp = 0, eventCounts = {}, recentActivity = 0 }) {
  ...
  const xpPart = Math.min(50, Number(xp) / 10); // 500 XP → full
  ...
  // Top activity types by count (for evidence).
  ...
  // Sparse high-value types: event keys with 0 count, ranked by XP desc.
...
/**
 * Compute all 8 Career Intelligence metrics in one pass.
 *
 * @param {object} input
 ...
 * @param {any[]} [input.learningTopics]
 * @param {any[]} [input.learningSessions]
 * @param {{ total?: number, byType?: object }} [input.memoryStats]
 * @param {object} [input.user] - { streak, lastActiveAt, xp }
 * @param {any[]} [input.timelineEvents] - need occurredAt
 * @param {{ topImprovement?: string|null }} [input.trends] - computeInterviewTrends result
 * @param {object} [input.eventCounts] - per-source counts for Productivity
 * @param {number} [input.recentActivity] - last-7-day activity count for Productivity
 * @param {string} [input.computedAt] - ISO timestamp (action supplies; Date.now is fine there)
 * @returns {{ computedAt: string, metrics: object }}
 */
export function computeIntelligence(input = {}) {
  ...
  // Timeline → distinct active-day timestamps for Consistency.
