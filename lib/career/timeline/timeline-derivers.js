...
/**
 * A learning topic → one or two milestones: a "learning" start event always,
 * plus an "achievement" mastered event when the topic reached `learned`.
 * Returns an array (a single topic can yield two distinct milestones, each
 * with its own `#started` / `#learned` sourceId so both survive dedup).
 *
 * @param {{ id: string, skill: string, status?: string, createdAt: Date|string, updatedAt?: Date|string }} topic
 * @returns {TimelineEventSpec[]}
 */
...
/**
 * A structured memory → a milestone for the two "milestone-shaped" categories:
 * `certificate` → "achievement", `project` → "building". Other categories
 * (skill/achievement/preference/resume_version/lesson/note) are not milestone
 * events here — they surface through other flows or aren't single milestones.
 *
 * @param {{ id: string, category: string, title: string, summary?: string|null, createdAt: Date|string }} mem
 * @returns {TimelineEventSpec|null}
 */
