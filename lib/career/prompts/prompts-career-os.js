/**
 * Prompt builders for the Career OS agentic + memory layer.
 *
 * Follows the exact tagged-template pattern of `lib/ai/prompts.js` — each
 * builder is a plain function returning a prompt string, so prompts stay in
 * one place and the same builder can be reused by a server action and an
 * Inngest job. The Prompt Service (`./prompt-service.js`) registers these by
 * id; agents call `buildPrompt(id, ctx)`.
 */
import { buildMemoryBlocks } from "@/lib/career/ui/citations";
import { buildChatContext } from "@/lib/career/ui/chat-context";

/**
 * Extract durable career facts from a chat turn (user message + the assistant
 * reply). Filters trivia ("hi", "thanks", "ok") and one-off chatter — only
 * facts worth remembering long-term (interviews scheduled, stated weaknesses,
 * preferences, goals, achievements, skills, application intents).
 *
 * Returns a JSON array of memory specs the Memory Service writes idempotently.
 */
...
// ---------------------------------------------------------------------------
// Coordinator + specialist agent prompts (M5).
//
// Each builder takes a `ctx` object assembled by the agent/coordinator and
// returns a prompt string. JSON-returning prompts end with the "Return ONLY
// this JSON" guard the rest of the codebase uses.
// ---------------------------------------------------------------------------

/**
 * Coordinator intent classification. Routes the user message to 1-3 specialist
 * agents. Returns intent + the agent ids to dispatch.
 */
...
/**
 * Specialist agent prompt — shared shape. `ctx.agentId`, `ctx.role`,
 * `ctx.task`, `ctx.context` (the buildChatContext promptText), `ctx.memory`.
 */
...
/**
 * Coordinator synthesis prompt — produces the final streamed reply that
 * combines the agent findings + recalled memory into one coherent, grounded
 * answer to the user. The caller streams this with generateTextStream.
 */
...
/** Memory-block helper re-exported so the route can build citation metadata. */
export { buildMemoryBlocks };

/**
 * Coach nudge prompt (M5 nudgeNow / M10 weekly digest). Generates 1-3 proactive
 * CoachInsight rows from the user's recent activity + memory.
 */
...
/**
 * Coach weekly digest prompt (M10). Same JSON shape as `coachNudgePrompt` but
 * framed as a weekly review: produces 3-5 CoachInsights covering the past
 * week's progress, risks, and the single highest-leverage next action. Called
 * by the Inngest `weekly-coach-digest` cron for every onboarded user.
 */
...
// ---------------------------------------------------------------------------
// GitHub Project Analyzer (M7).
// ---------------------------------------------------------------------------

/**
 * Senior-Engineer repository review. The Inngest `analyze-github-repo` job
 * fetches the repo metadata + file tree + README + a sample of key files, then
 * calls this prompt to produce a 6-section review the /github page renders in
 * SpotlightCards. Grounded strictly in the fetched content (no hallucinated
 * files).
 *
 * @param {{ fullName: string, description?: string, language?: string, defaultBranch?: string, tree: string[], readme?: string, files: Array<{path:string,content:string}> }} ctx
 */
...
/**
 * Build (or rebuild) the user's Career Twin profile from everything NovaNest
 * remembers: all non-forgotten memory grouped by type, the resume, connected
 * GitHub repos, the last few mock interviews, and tracked applications. The
 * result is persisted on `CareerTwin.profile` + bumps `User.twinVersion`.
 *
 * @param {{ profile: string, memories: any[], resume?: string, github?: any[], mocks?: any[], applications?: any[] }} ctx
 */
...
/**
 * Answer a question as the user's Career Twin — in the user's voice, grounded
 * in the built twin profile + a few recalled memories. Used by \`twinChat\`.
 *
 * @param {{ question: string, twin: any, memories?: any[] }} ctx
 */
